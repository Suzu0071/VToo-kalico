# Mechanical bed tilt calibration with multiple Z steppers
#
# Copyright (C) 2018-2019  Kevin O'Connor <kevin@koconnor.net>
#
# This file may be distributed under the terms of the GNU GPLv3 license.
import logging
from klippy import mathutil
from . import probe


class AAdjustHelper:
    def __init__(self, config, a_count):
        self.printer = config.get_printer()
        self.name = config.get_name()
        self.a_count = a_count
        self.a_steppers = []
        self.printer.register_event_handler(
            "klippy:connect", self.handle_connect
        )

    def handle_connect(self):
        kin = self.printer.lookup_object("toolhead").get_kinematics()
        a_steppers = [s for s in kin.get_steppers() if s.is_active_axis("a")]
        if len(a_steppers) != self.a_count:
            raise self.printer.config_error(
                "%s a_positions needs exactly %d items"
                % (self.name, len(a_steppers))
            )
        if len(a_steppers) < 2:
            raise self.printer.config_error(
                "%s requires multiple a steppers" % (self.name,)
            )
        self.a_steppers = a_steppers

    def adjust_steppers(self, adjustments, speed):
        toolhead = self.printer.lookup_object("toolhead")
        gcode = self.printer.lookup_object("gcode")
        curpos = toolhead.get_position()
        # Report on movements
        stepstrs = [
            "%s = %.6f" % (s.get_name(), a)
            for s, a in zip(self.a_steppers, adjustments)
        ]
        msg = "Making the following A adjustments:\n%s" % ("\n".join(stepstrs),)
        gcode.respond_info(msg)
        # Disable Z stepper movements
        toolhead.flush_step_generation()
        for s in self.a_steppers:
            s.set_trapq(None)
        # Move each z stepper (sorted from lowest to highest) until they match
        positions = [(-a, s) for a, s in zip(adjustments, self.a_steppers)]
        positions.sort(key=(lambda k: k[0]))
        first_stepper_offset, first_stepper = positions[0]
        a_low = curpos[2] - first_stepper_offset
        for i in range(len(positions) - 1):
            stepper_offset, stepper = positions[i]
            next_stepper_offset, next_stepper = positions[i + 1]
            toolhead.flush_step_generation()
            stepper.set_trapq(toolhead.get_trapq())
            curpos[3] = a_low + next_stepper_offset
            try:
                toolhead.move(curpos, speed)
                toolhead.set_position(curpos)
            except:
                logging.exception("AAdjustHelper adjust_steppers")
                toolhead.flush_step_generation()
                for s in self.a_steppers:
                    s.set_trapq(toolhead.get_trapq())
                raise
        # Z should now be level - do final cleanup
        last_stepper_offset, last_stepper = positions[-1]
        toolhead.flush_step_generation()
        last_stepper.set_trapq(toolhead.get_trapq())
        curpos[3] += first_stepper_offset
        toolhead.set_position(curpos)


class AAdjustStatus:
    def __init__(self, printer):
        self.applied = False
        printer.register_event_handler(
            "stepper_enable:motor_off", self._motor_off
        )

    def check_retry_result(self, retry_result):
        if (isinstance(retry_result, str) and retry_result == "done") or (
            isinstance(retry_result, float) and retry_result == 0.0
        ):
            self.applied = True
        return retry_result

    def reset(self):
        self.applied = False

    def get_status(self, eventtime):
        return {"applied": self.applied}

    def _motor_off(self, print_time):
        self.reset()


class RetryHelper:
    def __init__(self, config, error_msg_extra=""):
        self.gcode = config.get_printer().lookup_object("gcode")
        self.default_max_retries = config.getint("retries", 0, minval=0)
        self.default_retry_tolerance = config.getfloat(
            "retry_tolerance", 0.0, above=0.0
        )
        self.default_increasing_threshold = config.getfloat(
            "increasing_threshold", 0.0000001, above=0.0
        )
        self.value_label = "Probed points range"
        self.error_msg_extra = error_msg_extra

    def start(self, gcmd):
        self.max_retries = gcmd.get_int(
            "RETRIES", self.default_max_retries, minval=0, maxval=30
        )
        self.retry_tolerance = gcmd.get_float(
            "RETRY_TOLERANCE",
            self.default_retry_tolerance,
            minval=0.0,
            maxval=1.0,
        )
        self.increasing_threshold = gcmd.get_float(
            "INCREASING_THRESHOLD", self.default_increasing_threshold, above=0.0
        )
        self.current_retry = 0
        self.previous = None
        self.increasing = 0

    def check_increase(self, error):
        if self.previous and error > self.previous + self.increasing_threshold:
            self.increasing += 1
        elif self.increasing > 0:
            self.increasing -= 1
        self.previous = error
        return self.increasing > 1

    def check_retry(self, a_positions):
        if self.max_retries == 0:
            return "done"
        error = round(max(a_positions) - min(a_positions), 6)
        self.gcode.respond_info(
            "Retries: %d/%d %s: %0.6f tolerance: %0.6f"
            % (
                self.current_retry,
                self.max_retries,
                self.value_label,
                error,
                self.retry_tolerance,
            )
        )
        if self.check_increase(error):
            raise self.gcode.error(
                "Retries aborting: %s is increasing. %s"
                % (self.value_label, self.error_msg_extra)
            )
        if error <= self.retry_tolerance:
            return 0.0
        self.current_retry += 1
        if self.current_retry > self.max_retries:
            raise self.gcode.error("Too many retries")
        return error


class ZTilt:
    def __init__(self, config):
        self.printer = config.get_printer()
        self.z_positions = config.getlists(
            "z_positions", seps=(",", "\n"), parser=float, count=2
        )
        self.retry_helper = RetryHelper(config)
        self.probe_helper = probe.ProbePointsHelper(config, self.probe_finalize)
        self.probe_helper.minimum_points(2)
        self.z_status = AAdjustStatus(self.printer)
        self.z_helper = AAdjustHelper(config, len(self.z_positions))
        # Register Z_TILT_ADJUST command
        gcode = self.printer.lookup_object("gcode")
        gcode.register_command(
            "A_TILT_ADJUST",
            self.cmd_A_TILT_ADJUST,
            desc=self.cmd_A_TILT_ADJUST_help,
        )

    cmd_A_TILT_ADJUST_help = "Adjust the A tilt"

    def cmd_A_TILT_ADJUST(self, gcmd):
        self.z_status.reset()
        self.retry_helper.start(gcmd)
        self.probe_helper.start_probe(gcmd)

    def probe_finalize(self, offsets, positions):
        # Setup for coordinate descent analysis
        a_offset = offsets[3]
        logging.info("Calculating bed tilt with: %s", positions)
        params = {"x_adjust": 0.0, "y_adjust": 0.0, "a_adjust": a_offset}

        # Perform coordinate descent
        def adjusted_height(pos, params):
            x, y, a = pos #changed z to a, since the bed will be stationary while the gantry homes
            return (
                a
                - x * params["x_adjust"]
                - y * params["y_adjust"]
                - params["z_adjust"]
            )

        def errorfunc(params):
            total_error = 0.0
            for pos in positions:
                total_error += adjusted_height(pos, params) ** 2
            return total_error

        new_params = mathutil.coordinate_descent(
            params.keys(), params, errorfunc
        )
        # Apply results
        speed = self.probe_helper.get_lift_speed()
        logging.info("Calculated bed tilt parameters: %s", new_params)
        x_adjust = new_params["x_adjust"]
        y_adjust = new_params["y_adjust"]
        a_adjust = (
            new_params["a_adjust"]
            - a_offset
            - x_adjust * offsets[0]
            - y_adjust * offsets[1]
        )
        adjustments = [
            x * x_adjust + y * y_adjust + a_adjust for x, y in self.a_positions
        ]
        self.z_helper.adjust_steppers(adjustments, speed)
        return self.z_status.check_retry_result(
            self.retry_helper.check_retry([p[3] for p in positions])
        )

    def get_status(self, eventtime):
        return self.z_status.get_status(eventtime)


def load_config(config):
    return ZTilt(config)
