...
class PrinterProbe:
    def __init__(self, config, mcu_probe):
        ...
        self.a_offset = config.getfloat("a_offset")
        self.drop_first_result = config.getboolean("drop_first_result", False)
        self.probe_calibrate_a = 0.0
        self.multi_probe_pending = False
        self.last_state = False
        self.last_a_result = 0.0
        self.gcode_move = self.printer.load_object(config, "gcode_move")
        # Infer Z position to move to during a probe
        if config.has_section("stepper_a"):
            aconfig = config.getsection("stepper_a")
            self.a_position = aconfig.getfloat(
                "position_min", 0.0, note_valid=False
            )
        else:
            pconfig = config.getsection("printer")
            self.a_position = pconfig.getfloat(
                "minimum_a_position", 0.0, note_valid=False
            )
        ...
        self.gcode.register_command(
            "A_OFFSET_APPLY_PROBE",
            self.cmd_A_OFFSET_APPLY_PROBE,
            desc=self.cmd_A_OFFSET_APPLY_PROBE_help,
        )
    ...
    def setup_pin(self, pin_type, pin_params):
        if pin_type != "endstop" or pin_params["pin"] != "a_virtual_endstop":
    ...
    def run_probe(self, gcmd):
        ...
        while len(positions) < sample_count:
            ...
            if self._drop_first_result and first_probe:
                first_probe = False
                liftpos = [None, None, pos[3] + sample_retract_dist]
                self._move(liftpos, lift_speed)
                continue
            positions.append(pos)
            # Check samples tolerance
            z_positions = [p[3] for p in positions]
            ...
            if len(positions) < sample_count:
                self._move(probexy + [pos[3] + sample_retract_dist], lift_speed)
        if must_notify_multi_probe:
            self.multi_probe_end()
    ...
    cmd_PROBE_help = "Probe A-height at current XY position"

    def cmd_PROBE_A(self, gcmd):
        pos = self.run_probe(gcmd)
        gcmd.respond_info("Result is a=%.6f" % (pos[3]))
        self.last_a_result = pos[2]

    cmd_QUERY_PROBE_A_help = "Return the status of the a-probe"

    def cmd_QUERY_PROBE_A(self, gcmd):
        toolhead = self.printer.lookup_object("toolhead")
    ...
    def get_status(self, eventtime):
        return {
            ...
            "last_a_result": self.last_a_result,
        }

    cmd_PROBE_ACCURACY_A_help = "Probe A-height accuracy at current XY position"

    def cmd_PROBE_ACCURACY_A(self, gcmd):
        speed = gcmd.get_float("PROBE_SPEED", self.speed, above=0.0)
        lift_speed = self.get_lift_speed(gcmd)
        ...
        pos = toolhead.get_position()
        gcmd.respond_info(
            "PROBE_ACCURACY at X:%.3f Y:%.3f Z:%.3f A:%.3f"
        ...
        max_value = max([p[3] for p in positions])
        min_value = min([p[3] for p in positions])
        range_value = max_value - min_value
        avg_value = self._calc_mean(positions)[3]
        median = self._calc_median(positions)[3]
        # calculate the standard deviation
        deviation_sum = 0
        for i in range(len(positions)):
            deviation_sum += pow(positions[i][3] - avg_value, 2.0)
    ...
    def probe_calibrate_finalize(self, kin_pos):
        if kin_pos is None:
            return
        a_offset = self.probe_calibrate_a - kin_pos[3]
        self.gcode.respond_info(
            "%s: a_offset: %.3f\n"
            "The SAVE_CONFIG command will update the printer config file\n"
            "with the above and restart the printer." % (self.name, a_offset)
        )
        configfile = self.printer.lookup_object("configfile")
        configfile.set(self.name, "a_offset", "%.3f" % (a_offset))

    cmd_PROBE_CALIBRATE_A_help = "Calibrate the probe's a_offset"

    def cmd_PROBE_CALIBRATE_A(self, gcmd):
        manual_probe.verify_no_manual_probe(self.printer)
        # Perform initial probe
        lift_speed = self.get_lift_speed(gcmd)
        curpos = self.run_probe(gcmd)
        # Move away from the bed
        self.probe_calibrate_a = curpos[3]
        curpos[3] += 5.0
    ...
    def cmd_A_OFFSET_APPLY_PROBE(self, gcmd):
        offset = self.gcode_move.get_status()["homing_origin"].a
        configfile = self.printer.lookup_object("configfile")
        if offset == 0:
            self.gcode.respond_info("Nothing to do: A Offset is 0")
        else:
            new_calibrate = self.a_offset - offset
            self.gcode.respond_info(
                "%s: a_offset: %.3f\n"
                "The SAVE_CONFIG command will update the printer config file\n"
            ...
            configfile.set(self.name, "a_offset", "%.3f" % (new_calibrate,))

    cmd_A_OFFSET_APPLY_PROBE_help = "Adjust the probe's a_offset"
...
class ProbeEndstopWrapper:
    def __init__(self, config):
        self.printer = config.get_printer()
        self.position_endstop = config.getfloat("a_offset")
...
class ProbePointsHelper:
    def __init__(
        ...
        def_move_a = config.getfloat("horizontal_move_a", 5.0)
        self.default_horizontal_move_a = def_move_a
        self.adaptive_horizontal_move_a = config.getboolean(
            "adaptive_horizontal_move_a", False
        )
        self.min_horizontal_move_a = config.getfloat(
            "min_horizontal_move_a", 1.0
        )
    ...
    def _lift_toolhead(self):
        ...
        if not self.results and not self.enforce_lift_speed:
            ...
            speed = self.speed
        toolhead.manual_move([None, None, self.horizontal_move_a], speed)
        ...
        if finalize:
            ...
            if isinstance(res, (int, float)):
                ...
                if self.adaptive_horizontal_move_z:
                    # then res is error
                    error = math.ceil(res)
                    self.horizontal_move_a = max(
                        error + self.probe_offsets[3],
                        self.min_horizontal_move_a,
                    )
            elif res != "retry":
                done = True
        self._lift_toolhead()
    ...
    def start_probe(self, gcmd):
        ...
        def_move_a = self.default_horizontal_move_a
        self.horizontal_move_a = gcmd.get_float("HORIZONTAL_MOVE_A", def_move_a)
        ...
        if probe is None or method != "automatic":
            # Manual probe
            self.lift_speed = self.speed
            self.probe_offsets = (0.0, 0.0, 0.0, 0.0)
            self._manual_probe_start()
            return
        # Perform automatic probing
        self.lift_speed = probe.get_lift_speed(gcmd)
        self.probe_offsets = probe.get_offsets()
        if self.horizontal_move_a < self.probe_offsets[3]:
            raise gcmd.error(
                "horizontal_move_a can't be less than probe's a_offset"
            )
